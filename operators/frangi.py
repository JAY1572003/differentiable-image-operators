...
Kernel size is now FIXED at construction (derived from the max of sigma's
range), not computed from sigma.item() at runtime. sigma still shapes the
kernel VALUES (still differentiable) — it just never determines kernel SIZE.
"""
...
class DiffFrangi(Operation):
    def __init__(self, sigma=2.0, beta=0.5, gamma=5.0, black_ridges=True):
        ...
        # Fixed kernel size, chosen ONCE from the max of sigma's range.
        max_sigma = self.param_ranges['sigma'][1]
        k = int(6 * max_sigma + 1)
        if k % 2 == 0:
            k += 1
        self.kernel_size = k

    def _gauss1d(self, sigma, order):
        k = self.kernel_size
        ...
        if order == 0:
            ...
            d2g = (ax**2 / sigma**4 - 1.0 / sigma**2) * g
    ...
    def _eigenvalues(Hxx, Hxy, Hyy):
        ...
        ev1 = trace / 2.0 - disc
        ev2 = trace / 2.0 + disc
        # Standard Frangi convention: order by ABSOLUTE value, |lam1| <= |lam2|.
        # (Our previous version ordered by signed value, which silently picked
        # the wrong eigenvalue for the ridge test below and killed the response
        # exactly on real ridges.)
        swap = ev1.abs() > ev2.abs()
        lam1 = torch.where(swap, ev2, ev1)
        lam2 = torch.where(swap, ev1, ev2)
        return lam1, lam2
